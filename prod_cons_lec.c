...
void *producer(void *arg) {
	...
	while (1) {
        ...
        if(so->full == 1) pthread_cond_wait(&prod_cv, &so->lock); // full하면 consumer가 데이터 사용할 때까지 기다림
...
void *consumer(void *arg) {
	...
	while (1) {
        pthread_mutex_lock(&so->lock); // 이렇게 하면 unlock 안하고 코드를 빠져나감
		
        while(so->full == 0) pthread_cond_wait(&cons_cv, &so->lock);
		...
		i++;
		so->full = 0;
        pthread_cond_signal(&prod_cv); // data 소비하고 signal을 보냄
        pthread_mutex_unlock(&so->lock);
	}
	printf("Cons: %d lines\n", i);
	*ret = i;
